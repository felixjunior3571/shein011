...
COMMENT ON TABLE payment_webhooks IS 'Armazena webhooks de pagamento da SuperPay e outros gateways';
COMMENT ON COLUMN payment_webhooks.external_id IS 'ID externo único da fatura (ex: SHEIN_1751326765944_1qyme2zvx)';
COMMENT ON COLUMN payment_webhooks.status_code IS 'Código de status da SuperPay (5=pago, 12=negado, 15=vencido)';
COMMENT ON COLUMN payment_webhooks.is_paid IS 'TRUE quando status_code = 5 (pagamento confirmado)';
COMMENT ON COLUMN payment_webhooks.gateway IS 'Gateway de pagamento (superpaybr, tryplopay, etc)';
