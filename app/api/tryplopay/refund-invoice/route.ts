...
export async function PUT(request: NextRequest) {
  try {
    console.log("[TRYPLOPAY] === ESTORNANDO FATURA ===")
    ...
    const { id: invoiceId, amount } = body.payment || body
    ...
    if (!amount || amount <= 0) {
      return NextResponse.json(
        {
          success: false,
          error: "Valor do estorno é obrigatório e deve ser maior que zero",
        },
        { status: 400 },
      )
    }

    console.log("[TRYPLOPAY] Estornando fatura:", invoiceId, "Valor:", amount)
    ...
    const payload = {
      payment: {
        id: invoiceId,
        amount: Number(amount), // Valor em centavos conforme documentação
      },
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: `Erro ao estornar fatura: ${response.status}`,
          api_error: data,
        },
    ...
    console.log("[TRYPLOPAY] ✅ Fatura estornada com sucesso")
    ...
    return NextResponse.json({
      ...
      status: data.fatura?.status?.text || "refunded",
    })
  } catch (error) {
