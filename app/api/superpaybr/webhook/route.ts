...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("📦 Webhook SuperPay completo:", JSON.stringify(body, null, 2))
    ...
    // Extrair dados do webhook conforme logs fornecidos
    ...
    console.log("🎯 Dados extraídos do webhook SuperPay:", {
      ...
      return NextResponse.json(
        {
          ...
          message: `Status ${statusCode} não mapeado na documentação SuperPay`,
        },
        { status: 400 },
      )
    }
    ...
    console.log("💾 Salvando webhook SuperPay no banco:", {
    ...
    if (dbError) {
      console.error("❌ ERRO CRÍTICO ao salvar webhook SuperPay:", dbError)
        ...
        return NextResponse.json(
          {
            ...
            sql_script: "scripts/create-payment-webhooks-table.sql",
          },
          { status: 500 },
        )
      }
    ...
    console.log("✅ Webhook SuperPay salvo com SUCESSO no banco!")

    // Log específico para cada status importante
    if (statusCode === 5) {
    ...
    // Retornar resposta de sucesso para SuperPay
    const response = {
      success: true,
      status: "processed",
    ...
    console.log("✅ Resposta enviada para SuperPay:", response)
    console.log("🏁 Processamento webhook SuperPay CONCLUÍDO!\n")

    return NextResponse.json(response, { status: 200 })
  } catch (error) {
    console.error("❌ ERRO CRÍTICO NO WEBHOOK SUPERPAY:", error)
