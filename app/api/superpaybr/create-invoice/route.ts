...
export async function POST(request: NextRequest) {
  try {
    ...
    // Resposta no formato TryploPay com validação completa
    const response = {
      success: true,
      data: {
        ...
        valores: {
          bruto: Math.round(validAmount * 100) || 3490, // SEMPRE definido
          liquido: Math.round(validAmount * 100) || 3490, // SEMPRE definido
        },
        vencimento: {
    ...
    const fallbackQrCode = `https://quickchart.io/qr?text=${encodeURIComponent(fallbackPixPayload)}&size=250&format=png&margin=1`
